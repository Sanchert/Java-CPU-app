...
| Команда | Описание                     | Синтаксис                  | Пример      | Результат     | 
|:-------:|------------------------------|----------------------------|-------------|---------------|
|  INIT   | Инициализация ячейки памяти  | `INIT <адрес> <значение>`  | `INIT 10 5` | `mem[10] = 5` |
|   LD    | Загрузка из памяти в регистр | `LD <регистр> <адрес>`     | `LD A 10`   | `A = mem[10]` |
|   ST    | Сохранение регистра в память | `ST <адрес> <регистр>`     | `ST 20 B`   | `mem[20] = B` |
|   MV    | Копирование между регистрами | `MV <приёмник> <источник>` | `MV C A`    | `C := A;`     |

### Арифметические команды
| Команда | Описание  | Синтаксис | Результат         |
|:-------:|-----------|:---------:|-------------------|
|   ADD   | Сложение  |   `ADD`   | (`A` + `B` → `D`) |
|   SUB   | Вычитание |   `SUB`   | (`A` - `B` → `D`) |
|   MUL   | Умножение |   `MUL`   | (`A` * `B` → `D`) |
|   DIV   | Деление   |   `DIV`   | (`A` / `B` → `D`) |
...
|----------|--------------------------|------------|
| PRINT    | Вывод значений регистров | `PRINT`    |
